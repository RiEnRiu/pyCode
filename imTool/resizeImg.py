...
#from imBoost import imResizer, imgIO
import pyBoost
import imBoost
...
if __name__=='__main__':
    ...
    if(args.recursion!=0):
        file_relative = scanner.file_r(args.dir,'.jpg.png.jpeg',False,True)
    else:
        file_relative = scanner.file(args.dir,'.jpg.png.jpeg',False,True)

    if(args.save is None):
        read_img_list = [os.path.join(args.dir,x) for x in file_relative]
        save_img_list = read_img_list
    else:
        pyBoost.makedirs(args.save)
        #if(os.path.isdir(args.save) is not True):
        #    os.makedirs(args.save)
        read_img_list = [os.path.join(args.dir,x) for x in file_relative]
        save_img_list = [os.path.join(args.save,x) for x in file_relative]

    pimgio = imBoost.imgIO(read_img_list,True,100)
    pimRszr = imBoost.imResizer(wh,args.rtype,args.inter)    
    for i in range(len(read_img_list)):
        img = pimgio.read()
        rimg = pimRszr.imResize(img)
        pimgio.write(save_img_list[i],rimg)
    pimgio.waitEnd()
