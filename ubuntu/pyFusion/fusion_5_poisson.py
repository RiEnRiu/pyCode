...
if __name__ == '__main__':
    ...
    for i in tqdm(range(args.num),ncols=50):
        ...
        for ii in range(len(objs)):
            obj = np.zeros((objs[ii].shape[0],objs[ii].shape[1],4),np.uint8)
            obj[:,:,0:3] = objs[ii]
            obj[:,:,3] = masks[ii]
            objs[ii]=obj

        #resize
        objs, masks = resizeObjs(bg,objs,masks,labels,objSize,[0.6,0.7])
        ...
        #do fusion
        fusions = [bg.copy(),bg.copy(),bg.copy(),bg.copy(),bg.copy()]
        for obj, mask ,xmlobj in zip(objs,masks,p_vocXmlobj_list):
            ...
            obj_5 = [obj.copy(),obj.copy(),obj.copy(),obj.copy(),obj.copy()]
            
            obj_5[0][:,:,3] = 0
            fusions[0] = imFusion(obj_5[0],fusions[0],mask,cen_point,FUSION_TYPE.CV2_NORMAL_CLONE)

            for ii in range(1,5,1):
                obj_5[ii][:,:,3] = (obj_5[ii][:,:,3]*(ii*25.0/100.0)).astype(np.uint8)
                fusions[ii] = imFusion(obj_5[ii],fusions[ii],mask,cen_point,FUSION_TYPE.CV2_NORMAL_CLONE)
        #make vocXml
        p_vocXml = vocXml(objs=p_vocXmlobj_list)
        p_vocXml.width = fusions[0].shape[1]
        p_vocXml.height = fusions[0].shape[0]
        p_vocXml.folder='JPEGImages'

        for ii in range(5):
            fusion = fusions[ii]
            pp_vocXml = p_vocXml.copy()
            pp_vocXml.filename = 'fusion_{}_{}.jpg'.format(date,save_index)
            pp_vocXml.path = os.path.join(args.save,'JPEGImages',pp_vocXml.filename)
            if(p_rszr is not None):
                #print('resized')
                fusion = p_rszr.imResize(fusion)
                ret,pp_vocXml = p_rszr.vocXmlResize(pp_vocXml)   
           
            fusion = cv2.bilateralFilter(fusion,5,100,3)
            pImgIO.write(pp_vocXml.path,fusion)
            pVocXmlIO.write(os.path.join(args.save,'Annotations',\
                        'fusion_{}_{}.xml'.format(date,save_index)),pp_vocXml)

            save_index += 1
    print('finish and wait end...')
    pImgIO.waitEnd()
    pVocXmlIO.waitEnd()
